...
def softmax(z):
    ...
    s = np.max(z, axis=1)
    s = s[:, np.newaxis]  # ブロードキャストを行うために必要なステップ
    e_x = np.exp(z - s)
    div = np.sum(e_x, axis=1)
    div = div[:, np.newaxis]  # 同上
    return e_x / div
...
class SCRFD:
    ...
    def prepare(self, ctx_id, **kwargs):
        ...
        if input_size is not None:
            if self.input_size is not None:
                print("warning: scrfd モデルにはすでに det_size が設定されています。無視します。")
            else:
    ...
    def forward(self, img, thresh):
        ...
        for idx, stride in enumerate(self._feat_stride_fpn):
            # モデルがバッチ次元をサポートしている場合は、最初の出力を取得
            if self.batched:
            ...
            # モデルがバッチ処理をサポートしていない場合は、出力をそのまま取得
            else:
                scores = net_outs[idx]
    ...
    def detect(
        self, image, thresh=0.5, input_size=(640, 640), max_num=0, metric="default"
    ):
        ...
        if max_num > 0 and det.shape[0] > max_num:
            ...
            bindex = np.argsort(values)[::-1]  # センタリングに少し追加の重みを付与
